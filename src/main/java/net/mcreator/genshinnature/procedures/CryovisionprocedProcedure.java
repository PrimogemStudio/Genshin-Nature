...
@EventBusSubscriber
public class CryovisionprocedProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, Entity sourceentity) {
		if (entity == null || sourceentity == null)
			return;
		if (sourceentity instanceof LivingEntity _livEnt0 && _livEnt0.hasEffect(GenshinNatureModMobEffects.CRYOVISIONEFFECT)) {
			if (Math.random() > 0.9) {
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 60, 0));
			}
		}
		if (sourceentity instanceof LivingEntity _livEnt2 && _livEnt2.hasEffect(GenshinNatureModMobEffects.CRYODELUSIONEFFECT)) {
			if (Math.random() > 0.8) {
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
