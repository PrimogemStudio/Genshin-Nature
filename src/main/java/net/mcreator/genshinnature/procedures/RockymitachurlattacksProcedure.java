...
public class RockymitachurlattacksProcedure {
		...
		if (((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == GenshinNatureModBlocks.ELECTROCRYSTAL.get()) == false) {
			if (Math.random() < 0.1) {
				if (world instanceof ServerLevel _level) {
					Entity entityToSpawn = GenshinNatureModEntities.GEOSLIME.get().spawn(_level, BlockPos.containing(x, y + 1, z), MobSpawnType.MOB_SUMMONED);
